...
- **main** on GitHub includes Session 5 code (scoring engine, Telegram daemon, KV merge, plists, these notes).
- **LaunchAgents** installed under `~/Library/LaunchAgents/` and loaded with `launchctl load`: `com.steveonan.btc-scoring-engine` (hourly) + `com.steveonan.btc-telegram-bot` (`KeepAlive`).
...
**Clipboard helper** — in Terminal, run **only** these lines (do **not** paste instructional comment lines from chat; if zsh says `command not found: #`, it tried to execute a line starting with `#`):

```bash
cd "/Users/steveonan/Documents/Crypto Trading/TradingViewMCP/tradingview-mcp/pine"
./copy_to_clipboard.sh main
./copy_to_clipboard.sh rsi
./copy_to_clipboard.sh macd
```

After **each** command, paste into TradingView’s Pine Editor and **Add to chart**.
...
Reload plists after editing copies in the repo:
